...
const CreateArticle = ({ history, edit, slug }) => {
    ...
    const onSubmit = ({ articleTitle, articleDescription, articleText }) => {
        ...
        if (edit) {
            ...
            createArticle(articleTitle, articleDescription, articleText, tagList, token).then(
                ({ article: { slug } }) => {
                    history.push(`/articles/${slug}`)
                }
            )
    ...
    return (
        <div className={styles['create-article']}>
            <h3 className={styles['create-article__title']}>{edit ? 'Edit article' : 'Create new article'}</h3>
            <form className={styles['create-article__form']} onSubmit={handleSubmit(onSubmit)}>
                <label className={styles['create-article__label']}>
                    Title
                    <input
                        className={styles['create-article__input']}
                ...
                <div className={styles['tag-list']}>
                    <h4 className={styles['create-article__label']}>Tags</h4>
                    <TagList tags={tags} addTag={addTag} deleteTag={deleteTag} />
                </div>
                <button className={styles['create-article__submit']} type="submit">
                    Send
                </button>
            </form>
        </div>
...
const TagList = ({ tags, addTag, deleteTag }) => {
    ...
    return (
        <>
            {tags.map(({ id, label }) => {
                return (
                    <div key={id} className={styles['tag-list__item']}>
                        <div className={styles['tag-list__input']}>{label}</div>
                        <button className={styles['tag-list__delete']} onClick={() => deleteTag(id)}>
                            Delete
                        </button>
                    </div>
                )
            ...
            <div className={styles['tag-list__item']}>
                <input
                    className={styles['tag-list__input']}
                    placeholder="Tag"
                    onChange={(e) => setInputValue(e.target.value)}
                    value={inputValue}
                />
                <button className={styles['tag-list__delete']} onClick={clearInput}>
                    Delete
                </button>
                <button className={styles['tag-list__add']} onClick={onAddTag}>
                    Add tag
                </button>
            </div>
        </>
